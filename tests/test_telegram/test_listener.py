...
class TestProcessUpdate:
    ...
    async def test_busy_rejects_concurrent(
        self, listener: BotListener
    ) -> None:
        listener._busy = True
        update = {
        ...
        assert "working" in mock_send.call_args[0][0].lower() or \
               "wait" in mock_send.call_args[0][0].lower()
...
class TestRunLoop:
    async def test_stop_halts_loop(self, listener: BotListener) -> None:
        ...
        with patch.object(
            listener.client, "get_updates", side_effect=fake_get_updates
        ):
            await listener.run()
    ...
    async def test_polling_error_does_not_crash(
        ...
        with (
            patch.object(
                listener.client, "get_updates", side_effect=flaky_get_updates
            ),
    ...
    async def test_processes_updates_from_poll(
        self, listener: BotListener
    ) -> None:
        updates = [
            {
        ...
        with (
            patch.object(
                listener.client, "get_updates", side_effect=get_updates_once
            ),
            patch.object(
                listener.dispatcher, "dispatch", new_callable=AsyncMock
            ) as mock_dispatch,
